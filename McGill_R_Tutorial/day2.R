...
# Packages: This time, we'll use a couple of packages. Normally, we load all packages together at the top of the script (for practical reasons only). This way, it's easy to see where all packages are. Packages are like modules: each one is a collection of functions (among other things) that we can use. In a way, loading a package is like adding functions to R---this makes R a very powerful tool, since anyone can create a package. It's almost always the case that there's a package for everything you want to do.
...
# But maybe you want to have a specific folder IN your desktop, something more 'organized'. Let's create a new folder (directory) IN our desktop called 'Day2'. Then, let's set our working directory again, as we'll be working inside 'Day2'.
...
# Of course, you can create a folder and set your dir manually, using your mouse. But this might be easier/faster. Also, this is a useful way to automate folder creating in your script.
...
# Even though we'll use data frames most of the time, *data tables* are WAY better (and faster). Their syntax is a bit more intricate, but they're definitely better if you have a lot of data

# In fact, you can calculate how much time it takes to read a file (or do perform any other task) in R. Let's test this loading the Buckeye Corpus:
...
# Now, we've loaded our data and the file is stored in a variable called 'data'. Everything we do to this variable will NOT affect the original file. R loaded a 'copy' of tapped.txt, and that's it. If you delete something by mistake, don't worry: the original will always be there as far as you do not export a new txt file and overwrite it (we exported a csv file last time).
...
# Today, we will actually use one of the datasets included in R. More specifically, these data are in an R package called languageR—which, btw, is extremely useful for linguists, of course.
...
# ? is used to get help on a function OR element in the R language. You can't use it for variables you created, only for terms that have an intrinsic meaning in R (or in one of its packages).
...
# This is particularly useful if you have a large dataset, since it's a very economical way of looking at the variables in the file.
...
# If you recall, you can also use indices (slice notation) to take a look at the data. Let's say you want to check the first 10 rows of the fourth column:
...
# This says: Inside data, select the column SemanticClass and give me the first 10 items. Note that here you don't use a comma, since a single column only has one dimension.
...
# Another important thing to do right after you load your data is check whether your variables are coded correctly. R will assume numbers are numeric, but sometimes you want numbers to be a factor, for example. This will need to be corrected.
...
# This is a *very* useful way of looking at your data. Even before we plot things, it's nice to take a look at counts/% given certain variables, for example.
...
# Note that the order of the arguments does matter. The third argument here is the third dimension, so it's best to pick a factor that has fewer levels.
...
# You can see that the default is to calculate %s over *all* cells, which isn't that useful here. We can solve that: For rows, use 1; for cols, use 2.
...
# By saying legend.text = TRUE, we're telling R to add a legend to the plot. This is automatically set, given the structure of counts. However, you might want to add different labels:
...
# Let's say you want to know the average length of theme by RealizationOfRecipient. Perhaps the longer the theme, the more likely it is to realize the recipient as an NP:
